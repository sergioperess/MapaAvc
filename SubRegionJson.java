import java.security.KeyPair;

public class SubRegionJson extends Region {

    private Region mainRegion;
    private byte[] encryptedData;
    private String serializedData;

    public SubRegionJson(Region mainRegion) {
        this.mainRegion = mainRegion;
    }

    public Region getMainRegion() {
        return mainRegion;
    }

    public byte[] getEncryptedData() {
        return encryptedData;
    }

    public String getSerializedData() {
        return serializedData;
    }

    public void encryptAndSerializeData(KeyPair keyPair) {
        EncryptionThread encryptionThread = new EncryptionThread();
        JsonSerializationThread jsonSerializationThread = new JsonSerializationThread();
        encryptionThread.setRegion(this);
        jsonSerializationThread.setRegion(this);
        encryptionThread.start();
        try {
            encryptionThread.join(); // Aguardar a conclusão da criptografia
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        jsonSerializationThread.start();
        try {
            jsonSerializationThread.join(); // Aguardar a conclusão da serialização JSON
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    private class EncryptionThread extends Thread {
        private SubRegionJson subRegion;

        public void setRegion(SubRegionJson subRegion) {
            this.subRegion = subRegion;
        }

        @Override
        public void run() {
            try {
                // Criptografar os dados da SubRegião
                byte[] encryptedData = encryptRSA(subRegion.toString().getBytes(), keyPair.getPrivate());
                subRegion.encryptedData = encryptedData;
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    }

    private class JsonSerializationThread extends Thread {
        private SubRegionJson subRegion;

        public void setRegion(SubRegionJson subRegion) {
            this.subRegion = subRegion;
        }

        @Override
        public void run() {
            try {
                // Serializar a SubRegião para formato JSON
                String json = subRegion.toJson();
                subRegion.serializedData = json;
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    }
}
